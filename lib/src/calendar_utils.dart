...
class CalendarUtils {
  static final List<String> weekDays = ['Th 2', 'Th 3', 'Th 4', 'Th 5', 'Th 6', 'Th 7', 'CN'];
  static final List<int> monthDays = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
  ...
  List<CalendarModel> getMonthCalendar(int month, int year,
    ...
    List<CalendarModel> calendar = List<CalendarModel>();
    ...
    // month-1 because monthDays starts from index 0 and month starts from 1
    ...
    if ((startWeekDay == StartWeekDay.sunday &&
      ...
      if (month == DateTime.january) {
        otherMonth = DateTime
            .december; // monthDays index starts from 0 (11 for december)
        otherYear = year - 1;
      } else {
        otherMonth = month - 1;
        otherYear = year;
  ...
  static int calculateDifferenceNow(DateTime date) {
